...
from macroway import is_row_valid_for_pitlane
...
def get_condition_mask(df: pd.DataFrame) -> pd.Series:
    ...
    return working_df.apply(is_row_valid_for_pitlane, axis=1)
        ...
        if matched_mask.any():
            ...
            for _, run_df in df[matched_mask].groupby(run_id[matched_mask]):
                ...
                ax.plot(
                    ...
                    label=f"{column} pit" if first_label else None,
                )
                first_label = False

    ax.set_title(f"{source}: pitot_c, pair, and carspeed_art with pit parts highlighted")
...
def plot_source_xy(ax, df: pd.DataFrame, source: str) -> None:
    ...
    ax.set_title(f"{source}: pair vs pitot_c on pit-only rows")
