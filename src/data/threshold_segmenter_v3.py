...
class MacroSegmenterV3:
    def __init__(self, config: SegmenterConfigV3 | None = None):
        self.cfg = config or SegmenterConfigV3()
    ...
    def _original_pit_mask(self, out: pd.DataFrame, dt: float) -> np.ndarray:
        c = self.cfg
        ...
        if c.apply_internal_smoothing:
            ...
            accx_smooth = out[c.accx_col].astype(float).rolling(sw, center=True, min_periods=1).median()
            accy_smooth = out[c.accy_col].astype(float).rolling(sw, center=True, min_periods=1).median()
        else:
    ...
    def fit_transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        pit = self._original_pit_mask(out, dt)

        moving = out["speed_smooth"] > c.pit_speed_max
        base = out["stable_speed"] & moving
        if int(base.sum()) < 100:
            base = moving
        ...
        th = {
            "straight_accy_max": float(aa[base].quantile(0.44)),
            "straight_yaw_max": float(ay[base].quantile(0.44)),
            "straight_steer_max": float(st[base].quantile(0.50)),
            "corner_accy_min": float(aa[base].quantile(0.50)),
            "corner_yaw_min": float(ay[base].quantile(0.50)),
            "corner_steer_min": float(st[base].quantile(0.52)),
        }
        self.thresholds_ = th
        ...
        straight = (
            ...
            & ~pit
            & ((aa <= th["straight_accy_max"]) | (ay <= th["straight_yaw_max"]))
        ...
        out["segment_macro_v3"] = labels
        out["segment_id_macro_v3"] = self._segment_ids(labels)
        self.debug_stats_ = {
            "input_row_count": input_row_count,
            "stable_speed_count": stable_speed_count,
            "counts_per_label": {
                str(label): int(count)
                for label, count in out["segment_macro_v3"]
    ...
    def segments_table(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for sid, g in df.groupby("segment_id_macro_v3", sort=True):
            lab = g["segment_macro_v3"].iloc[0]
            if lab == "transition":
                continue
            rows.append(
                {
                    "segment_id_macro_v3": sid,
                    "segment": lab,
...
def get_source_paths(source: str) -> tuple[Path, Path, Path, Path]:
    if source == "simu":
        ...
        return (
            cleaned_simudata_full_run_file(),
            base_dir / "barcelona_2026_simudata_segmented_macro_v3_run_46.csv",
            base_dir / "barcelona_2026_simudata_segment_summary_macro_v3_run_46.csv",
            base_dir / "barcelona_2026_simudata_segment_debug_macro_v3_run_46.json",
        )

    base_dir = processed_run_dir() / "Segmented"
    return (
        cleaned_merged_full_run_file(),
        base_dir / "barcelona_2026_merged_cleaned_macro_v3_run_46_segmented.csv",
        base_dir / "barcelona_2026_merged_cleaned_segment_summary_macro_v3_run_46.csv",
        base_dir / "barcelona_2026_merged_cleaned_segment_debug_macro_v3_run_46.json",
    )


def run_segmentation(
    source: str,
) -> tuple[pd.DataFrame, pd.DataFrame, MacroSegmenterV3, Path, Path, Path]:
    input_path, output_path, summary_path, debug_path = get_source_paths(source)
    df = pd.read_csv(input_path, low_memory=False)
    seg = MacroSegmenterV3(
        SegmenterConfigV3(
    ...
    debug_payload = {
        ...
        "stable_speed_count": stable_speed_count,
        "thresholds": {
            key: float(value) for key, value in seg.thresholds_.items()
        },
...
def plot_segmentation(df: pd.DataFrame, source: str) -> None:
    ...
    for label, color in color_map.items():
        mask = df["segment_macro_v3"] == label
        if mask.any():
            ax.scatter(x[mask], speed[mask], color=color, s=4, label=label)
    ax.set_title(f"Threshold segmenter v3 - {source}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply threshold segmenter v3 logic to our telemetry.",
    )
...
def main() -> None:
    ...
    print(out["segment_macro_v3"].value_counts(dropna=False))
